...
2. **Technical plan** — which components to edit or create, what changes to make in code
...
- Before asking to commit, update any `.claude/` files affected by the changes:
  - `features.md` — if a feature was added, changed, or scoped out
  - `schema.md` — if tables, columns, or RLS policies changed (run `make types` too)
  - `api-routes.md` — if server routes or Supabase calls were added or modified
