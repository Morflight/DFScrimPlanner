...
|-----------|----------|---------|
| AvailabilityGrid | `lib/components/AvailabilityGrid.svelte` | 7-day × 48-slot clickable grid; click+drag to select; emits `onchange(Set<string>)` with slot keys (`"YYYY-MM-DDTHH:MM"` in user tz); accepts `readonly={true}` to disable interaction; accepts `scrimSlots?: Set<string>` to overlay scheduled scrims as non-interactive amber diagonal stripes that break contiguous-run calculations |
...
| WeekNav | `lib/components/WeekNav.svelte` | Shared week navigation bar (← Prev / label / Next → / Today). Props: `weekLabel: string`, `weekOffset: number`, `onnavigate: (delta: number) => void`. Used on availability, team, and scrims pages |
